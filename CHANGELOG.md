...
### Changes
- VS Code workspace tasks now include a single deploy action that ships the current plugin ZIP to TranslatePress, WP-Globus, and SlyBase in sequence.
...
- TranslatePress frontend editor no longer shows a success banner after a completed run and now uses German copy consistently across the visible panel texts.
- TranslatePress frontend editor now refreshes its post-context automatically after in-app navigation, so the warning from parent pages clears itself when you switch to an editable singular post or page.
- TranslatePress frontend editor now translates only the currently visible TranslatePress text field instead of triggering a whole-post translation run from that UI.
- TranslatePress frontend editor model dropdown now uses a side-panel-like control style with the same taller field height, clearer focus state, and aligned refresh button.
- Inline text and block translation requests now clear stale cancel flags before they start, so a previously cancelled post job no longer aborts the next short TranslatePress string run immediately.
- TranslatePress string-mode translation no longer depends on singular post detection, so the panel stays usable without the old unsupported-context warning while users manually translate individual visible strings.
- Plain-text recovery prompts now preserve brace-style placeholders like `{current_year}` and `{theme_author}` explicitly, so variable-heavy TranslatePress strings no longer fall from passthrough into assistant-style explanations.
- Short text translation prompts now explicitly preserve template tokens and tell the model to translate only the remaining source-language fragments when a UI string is already partly in the target language.
- TranslatePress string-mode field detection now resolves source and target inputs by language hints instead of blindly using the first and last visible fields, so already-English target text is no longer resent as the source string.
- TranslatePress string-mode field detection now also prefers TranslatePress readonly inputs as the source string and editable inputs as the target, so mixed-language editor layouts no longer send the editable target text back as source input.
- TranslatePress string-mode field detection now scopes itself to the active visible TranslatePress string container and logs the chosen source and target field details, making wrong-field selections directly traceable.
- TranslatePress string-mode source resolution now prefers the active TranslatePress runtime string state's `original` value over the rendered readonly input, avoiding cases where the UI already shows a different language variant in the readonly field.
- TranslatePress string-mode start runs no longer break on the new source-resolution diagnostics, so the sidebar's Übersetzen button remains clickable while logging the chosen source details.
- TranslatePress string-mode source resolution now ranks visible candidates by source-language markers before falling back to readonly or position-based selection, so mixed-language field groups can prefer the actual German source text over an already-English preview field.
- TranslatePress sidebar warning banners now map the language-passthrough validation error to German UI copy instead of showing the backend's English fallback text.
...
- Decode failures now log detailed diagnostics including `object_count_hint`, `ends_with_brace`, and an `output_excerpt`, making the root cause immediately visible in timing logs.
...
- Content translation now batches eligible simple list items inside single `core/list` wrappers into one JSON-based model call, reducing list-related subcall fan-out while keeping the existing fallback path for complex lists.
- List-item batching now accepts richer `<li>` HTML content and logs deterministic `content_list_batch_skip` reasons when a list is not batch-eligible, making the next speed bottlenecks directly observable in timing logs.
- Content translation now attempts JSON micro-batching for groups of short non-list wrapper blocks, reducing tiny per-block model requests in content-heavy posts.
- Mixed `core/list` blocks now batch only flat list items and recurse only nested items, preserving structure while still collapsing most list-item calls into one request.
...
- UI: pressing Escape now closes the list-table translation picker on post and page overview screens.
...
- Reliability: OpenRouter Nemotron requests now forward reasoning-disable and provider-parameter requirements through the WP AI Client connector, making unsupported provider paths fail fast instead of silently ignoring the requested thinking controls.
...
- Validation: short German titles and headings are now rejected when an English-target translation returns them unchanged, and batched title/meta results are revalidated before reuse.
- Validation: title passthrough failures now retry once without the title-specific hint, and passthrough errors now describe the requested target language instead of always saying German.
