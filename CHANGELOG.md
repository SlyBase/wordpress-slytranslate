...
- Reliability: the plain-prompt recovery now uses a completion-style bilingual format (`DE: {text}\nEN:`) instead of an instruction-style prompt; this eliminates the remaining echo failures where Nemotron paraphrased the instruction text in its thinking output rather than producing a translation.- Polylang: translation creation no longer fails when the new target draft already has the requested language.
