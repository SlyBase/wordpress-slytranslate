...
- Reliability: when a model echoes the system prompt on both the initial attempt and the retry (both fail as `invalid_translation_assistant_reply`), a third recovery attempt now uses a plain user-only prompt without a system role; this unblocks Nemotron-family models that enter thinking mode on every system-prompted request.- Polylang: translation creation no longer fails when the new target draft already has the requested language.
