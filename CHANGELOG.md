...
### Changes
- Transport: unified all chat-capable LLM model families through the WordPress AI Client connector. The in-plugin Direct API transport (`DirectApiTranslationClient`) has been removed.
- Refactor: default model profile definitions were extracted into a dedicated registry class while keeping the existing profile filter contract unchanged.
- Transport: TranslateGemma now routes via the WP AI Client connector with `chat_template_kwargs` (`source_lang_code`/`target_lang_code`) injected through the existing connector-agnostic `http_request_args` filter.
...
- Profiles: model families now inherit from one universal default profile with sparse overrides, reducing maintenance for newly added model slugs.
- Profiles: Nemotron matching now follows the model family across providers instead of relying on OpenRouter-specific labels.

### Fixes
- i18n: added the missing German translation for the list-table overwrite confirmation warning.
- Validation: leaked retry directives like "Return only EN. Do not copy sentences in DE." are now rejected so prompt instructions are not saved as translated titles.
- UI: list-table model picker swap/refresh icons are now centered reliably, and the model-select row now uses the full available dialog width.
- OpenRouter Nemotron requests now normalize pasted label-style model slugs to the real connector id, use standard system/user chat prompts, and retry title translations once without the title-specific hint when the model answers with prompt-leaking assistant text.
- Transport: generic connector 502/upstream errors now surface nested provider details when the connector includes them in the error payload.
...
- MCP: `set-post-language` is now hidden from discovery when the active language plugin cannot mutate post-language assignments.
...
- Reliability: thinking-related `chat_template_kwargs` are now applied as best effort, with an automatic retry path that drops kwargs when an endpoint rejects them.
