...
## [1.6.0]
- German tone control: added a DE formality guardrail for explicit informal requests (e.g. "du statt Sie"). Outputs that still use formal address ("Sie"/"Ihnen"/"Ihr") now fail validation with a retryable `invalid_translation_formality_mismatch` error.
- Prompting: bilingual-frame requests now inject an explicit German informal-address style requirement when the prompt asks for "du" tone, and retries reinforce this requirement.
