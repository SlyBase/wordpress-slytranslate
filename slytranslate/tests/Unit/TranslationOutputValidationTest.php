...
class TranslationOutputValidationTest extends TestCase {
	...
	public function test_rejects_german_rewrite_when_target_is_english(): void {
		$source_text = 'Die Konfiguration ist abgeschlossen. WordPress 7, SlyTranslate, Polylang, MCP. Übersetzungen funktionieren mit einem Klick.';
		$translated  = 'Die Konfiguration ist abgeschlossen und die Übersetzungen funktionieren mit einem Klick. Als Nächstes plane ich automatisierte Workflows und weitere Dokumentation.';
	...
	public function test_language_passthrough_error_message_uses_target_language_direction(): void {
		$result = $this->invokeStatic(
			TranslationValidator::class,
			'validate',
			array(
				'WordPress 7 + LLM = kostenlose Übersetzung',
				'WordPress 7 + LLM = kostenlose Übersetzung',
				'en'
			)
		);
		...
		$this->assertInstanceOf( \WP_Error::class, $result );
		$this->assertStringContainsString( 'instead of English', $result->get_error_message() );
		$this->assertStringNotContainsString( 'instead of German', $result->get_error_message() );
	}
	...
	public function test_translate_chunk_plain_prompt_recovery_preserves_brace_placeholders(): void {
		$call_count = 0;
		$inputs     = array();
		...
		$source_text = 'Copyright © {current_year} - WordPress Theme von {theme_author}';
			...
			static function ( string $text ) use ( &$call_count, &$inputs, $source_text ) {
				return new class( $text, $call_count, $inputs, $source_text ) {
					...
					public function generate_text(): string {
						...
						return 'Copyright © {current_year} - WordPress theme by {theme_author}';
					}
				};
			}
		);
		...
		$this->assertSame( 'Copyright © {current_year} - WordPress theme by {theme_author}', $result );
		$this->assertCount( 3, $inputs );
		$this->assertStringContainsString( 'Preserve every placeholder token exactly as written.', $inputs[2] );
		$this->assertStringContainsString( '{current_year}', $inputs[2] );
		$this->assertStringContainsString( '{theme_author}', $inputs[2] );
	}
