...
class HookRegistrationTest extends TestCase {
	...
	public function test_add_hooks_registers_expected_editor_and_ability_hooks(): void {
		...
		$this->stubWpFunction(
			'add_action',
			static function ( string $hook, $callback, int $priority = 10, int $accepted_args = 1 ) use ( &$registered_hooks ): void {
				$registered_hooks[] = array(
					'type'          => 'action',
					'hook'          => $hook,
					'callback'      => $callback,
		...
		$this->assertSame(
			array(
				...
				array(
					'type'          => 'action',
					'hook'          => 'trp_translation_manager_footer',
					'callback'      => array( TranslatePressEditorIntegration::class, 'register_assets' ),
					'priority'      => 1,
					'accepted_args' => 1,
				),
				array(
					'type'          => 'filter',
					'hook'          => 'trp-scripts-for-editor',
					'callback'      => array( TranslatePressEditorIntegration::class, 'include_editor_script' ),
					'priority'      => 10,
					'accepted_args' => 1,
				),
				array(
					'type'          => 'filter',
					'hook'          => 'trp-styles-for-editor',
					'callback'      => array( TranslatePressEditorIntegration::class, 'include_editor_style' ),
					'priority'      => 10,
					'accepted_args' => 1,
				),
				array(
					'type'          => 'filter',
					'hook'          => 'trp_editor_data',
					'callback'      => array( TranslatePressEditorIntegration::class, 'inject_editor_data' ),
					'priority'      => 10,
					'accepted_args' => 1,
				),
				array(
		...
		$this->assertNotContains( 'plugins_loaded', array_column( $registered_hooks, 'hook' ) );
	}
}
