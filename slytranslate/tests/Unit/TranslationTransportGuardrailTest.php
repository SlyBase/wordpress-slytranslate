...
class TranslationTransportGuardrailTest extends TestCase {
	...
	public function test_wp_ai_client_curl_timeout_falls_back_to_direct_api_for_non_strict_models(): void {
		$wp_calls          = 0;
		$direct_endpoint   = '';
		$direct_body       = array();
		...
		$this->stubWpFunction( 'wp_remote_post',
			static function ( string $endpoint, array $args ) use ( &$direct_endpoint, &$direct_body ) {
				$direct_endpoint = $endpoint;
				$decoded         = json_decode( (string) ( $args['body'] ?? '' ), true );
				$direct_body     = is_array( $decoded ) ? $decoded : array();
		...
		$this->assertSame( 'Hallo Welt', $result );
		$this->assertSame( 1, $wp_calls, 'WP AI Client transport must be tried exactly once.' );
		$this->assertSame( 'http://llama.local:8080/v1/chat/completions', $direct_endpoint );
		$this->assertSame( 'Qwen3.5-9B-Q4_K_M', $direct_body['model'] ?? null );

		$diagnostics = $this->getStaticProperty( TranslationRuntime::class, 'last_diagnostics' );
		$this->assertSame( 'direct_api', $diagnostics['transport'] );
	}

	public function test_wp_ai_client_transport_failure_does_not_fallback_when_direct_api_url_missing(): void {
		$direct_calls = 0;
		...
		$this->stubWpFunction( 'wp_remote_post',
			static function () use ( &$direct_calls ) {
				++$direct_calls;
				throw new \RuntimeException( 'Direct API must not be invoked when no direct_api_url is configured.' );
			}
		);
	...
	public function test_wp_ai_client_authentication_error_does_not_trigger_direct_api_fallback(): void {
		...
		$this->stubWpFunction( 'wp_ai_client_prompt',
			static function () {
				return new class {
					...
					public function using_max_tokens( int $max_tokens ) { return $this; }
					public function generate_text() {
						return new \WP_Error( 'unauthorized', 'Invalid API key' );
					}
				};
			}
		);

		// Track translation chat-completions calls. Probe-only requests
		// (kwargs detection during preflight) are short-circuited with a
		// WP_Error so they don't count, but a real translation request
		// would carry our user content "Hello world" — flag that.
		$this->stubWpFunction( 'wp_remote_post',
			static function ( $endpoint, $args ) use ( &$chat_completion_calls ) {
				$body = is_string( $args['body'] ?? null ) ? $args['body'] : '';
				if ( false !== strpos( $body, 'Hello world' ) ) {
		...
		$this->assertSame( 0, $chat_completion_calls, 'Direct API must not retry the translation for auth-level errors.' );
	}
}
