...
class AI_Translate {
	...
	private static function get_editor_bootstrap_data(): array {
		return array(
			...
			'strings'              => array(
				'panelTitle'                => __( 'AI Translation with SlyTranslate', 'ai-translate-abilities' ),
				'sourceLanguageLabel'       => __( 'Source language', 'ai-translate-abilities' ),
				'targetLanguageLabel'       => __( 'Target language', 'ai-translate-abilities' ),
				'overwriteLabel'            => __( 'Overwrite existing translation', 'ai-translate-abilities' ),
				'translateTitleLabel'       => __( 'Translate title', 'ai-translate-abilities' ),
				'translateButton'           => __( 'Translate now', 'ai-translate-abilities' ),
				'refreshButton'             => __( 'Refresh translation status', 'ai-translate-abilities' ),
				'loadingLanguages'          => __( 'Loading available languages...', 'ai-translate-abilities' ),
				'loadingStatus'             => __( 'Loading translation status...', 'ai-translate-abilities' ),
				'noLanguages'               => __( 'No target languages are available for this content item.', 'ai-translate-abilities' ),
				'translationStatusLabel'    => __( 'Translation status', 'ai-translate-abilities' ),
				'translationExists'         => __( 'Available', 'ai-translate-abilities' ),
				'translationMissing'        => __( 'Not translated yet', 'ai-translate-abilities' ),
				'openTranslation'           => __( 'Open translation', 'ai-translate-abilities' ),
				'saveFirstNotice'           => __( 'Save the content before creating a translation.', 'ai-translate-abilities' ),
				'saveChangesNotice'         => __( 'Save your latest changes before translating so the translation uses the current content.', 'ai-translate-abilities' ),
				'translationCreatedNotice'  => __( 'Translation created successfully.', 'ai-translate-abilities' ),
				'translationUpdatedNotice'  => __( 'Translation updated successfully.', 'ai-translate-abilities' ),
				'existingTranslationNotice' => __( 'A translation already exists for the selected language. Enable overwrite to update it.', 'ai-translate-abilities' ),
				'translateSelectionButton'  => __( 'Translate selection', 'ai-translate-abilities' ),
				'translateSelectionTitle'   => __( 'Translate selected text', 'ai-translate-abilities' ),
				'translateSelectionTextLabel' => __( 'Selected text', 'ai-translate-abilities' ),
				'translateSelectionMissingSelection' => __( 'Select text in a paragraph, heading, or another text field first.', 'ai-translate-abilities' ),
				'translateSelectionUnavailable' => __( 'No target languages are available for the selected text.', 'ai-translate-abilities' ),
				'cancelButton'              => __( 'Cancel', 'ai-translate-abilities' ),
				'unknownError'              => __( 'An unexpected error occurred.', 'ai-translate-abilities' ),
			),
		);
	}
	...
	public static function register_editor_rest_routes() {
		...
		self::register_editor_rest_route( '/ai-translate/translate-post', array( static::class, 'rest_execute_translate_post' ) );
	}
	...
	public static function rest_execute_translate_post( \WP_REST_Request $request ) {
		return self::execute_translate_post( self::get_editor_rest_input( $request ) );
	}
	...
	private static function register_get_translation_status_ability() {
		wp_register_ability( 'ai-translate/get-translation-status', array(
			...
			'description'         => __( 'Returns the translation status for a post, page, or custom post type entry.', 'ai-translate-abilities' ),
			'category'            => 'ai-translation',
			'input_schema'        => array(
				'type'       => 'object',
				'properties' => array(
	...
	public static function execute_get_translation_status( $input ) {
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new \WP_Error( 'forbidden_post', __( 'You are not allowed to inspect this content item.', 'ai-translate-abilities' ) );
		}
	...
	private static function register_get_untranslated_ability() {
		wp_register_ability( 'ai-translate/get-untranslated', array(
			'label'               => __( 'Get Untranslated Content', 'ai-translate-abilities' ),
			'description'         => __( 'Lists posts, pages, or custom post types that do not yet have a translation in the requested language.', 'ai-translate-abilities' ),
			'category'            => 'ai-translation',
			'input_schema'        => array(
				'type'       => 'object',
				'properties' => array(
	...
	public static function execute_get_untranslated( $input ) {
		...
		if ( '' === $target_language ) {
			return new \WP_Error( 'missing_target_language', __( 'Target language is required.', 'ai-translate-abilities' ) );
		}
	...
	private static function register_translate_post_ability() {
		wp_register_ability( 'ai-translate/translate-post', array(
			...
			'description'         => __( 'Translates a post, page, or custom post type entry and creates or updates the translation.', 'ai-translate-abilities' ),
			'category'            => 'ai-translation',
			'input_schema'        => array(
				'type'       => 'object',
				'properties' => array(
	...
	private static function register_translate_posts_ability() {
		wp_register_ability( 'ai-translate/translate-posts', array(
			...
			'description'         => __( 'Translates multiple posts, pages, or custom post type entries. Continues on individual failures.', 'ai-translate-abilities' ),
			'category'            => 'ai-translation',
			'input_schema'        => array(
				'type'       => 'object',
				'properties' => array(
		...
		foreach ( $post_ids as $post_id ) {
			if ( ! current_user_can( 'edit_post', $post_id ) ) {
				...
				$results[] = array(
					...
					'status'             => 'failed',
					'error'              => __( 'You are not allowed to translate this content item.', 'ai-translate-abilities' ),
					'edit_link'          => '',
				);
				continue;
			}
			...
			if ( '' !== $source_language && $source_language === $target_language ) {
				...
				$results[] = array(
					...
					'translated_post_id' => 0,
					'status'             => 'skipped',
					'error'              => __( 'The source content is already in the requested target language.', 'ai-translate-abilities' ),
					'edit_link'          => '',
				);
				continue;
			}
			...
			if ( $existing_translation > 0 && ! $overwrite ) {
				...
				$results[] = array(
					...
					'status'             => 'skipped',
					'error'              => __( 'A translation already exists for the requested language.', 'ai-translate-abilities' ),
	...
	private static function register_configure_ability() {
		wp_register_ability( 'ai-translate/configure', array(
			...
			'description'         => __( 'Read or update AI Translate settings, including prompt template, meta keys, SEO defaults, and auto-translate behavior.', 'ai-translate-abilities' ),
			'category'            => 'ai-translation',
			'input_schema'        => array(
				'type'       => 'object',
				'properties' => array(
	...
	private static function get_translation_runtime_context(): array {
		...
		try {
			...
			$model = $service->get_model(
				array(
					'feature'      => 'ai-translate-abilities',
					'capabilities' => array( 'text_generation' ),
				)
			);
	...
	private static function translate_post_content( string $content, string $to, string $from = 'en' ) {
		if ( '' === trim( $content ) ) {
			return '';
		}

		if ( ! function_exists( 'parse_blocks' ) || ! function_exists( 'serialize_blocks' ) ) {
			return self::translate( $content, $to, $from );
		}
		...
		return self::translate_block_sections( $blocks, $to, $from );
	}

	private static function translate_block_sections( array $blocks, string $to, string $from = 'en' ) {
		$translated_sections = array();
		$pending_blocks      = array();

		foreach ( $blocks as $block ) {
			if ( self::should_skip_block_translation( $block ) ) {
				$translated_section = self::translate_serialized_blocks( $pending_blocks, $to, $from );
	...
	private static function translate_serialized_blocks( array $blocks, string $to, string $from = 'en' ) {
		if ( empty( $blocks ) ) {
			return '';
		}
		...
		return self::translate( $serialized_blocks, $to, $from );
	}

	private static function translate_parsed_blocks( array $blocks, string $to, string $from = 'en' ) {
		$translated_blocks = array();

		foreach ( $blocks as $block ) {
			$translated_block = self::translate_parsed_block( $block, $to, $from );
	...
	private static function translate_parsed_block( array $block, string $to, string $from = 'en' ) {
		if ( self::should_skip_block_translation( $block ) ) {
			return $block;
		}

		if ( isset( $block['innerBlocks'] ) && is_array( $block['innerBlocks'] ) && ! empty( $block['innerBlocks'] ) ) {
			$translated_inner_blocks = self::translate_parsed_blocks( $block['innerBlocks'], $to, $from );
		...
		if ( isset( $block['innerContent'] ) && is_array( $block['innerContent'] ) ) {
			foreach ( $block['innerContent'] as $index => $fragment ) {
				...
				$translated_fragment = self::translate( $fragment, $to, $from );
	...
	private static function resolve_bulk_source_post_ids( array $input ) {
		...
		if ( '' === $post_type ) {
			return new \WP_Error( 'missing_post_selection', __( 'Provide either post_ids or a post_type to translate in bulk.', 'ai-translate-abilities' ) );
		}
	...
	private static function validate_translatable_post_type( string $post_type ) {
		if ( '' === $post_type || ! post_type_exists( $post_type ) ) {
			return new \WP_Error( 'invalid_post_type', __( 'The requested post type does not exist.', 'ai-translate-abilities' ) );
		}
		...
		if ( function_exists( 'pll_is_translated_post_type' ) && ! pll_is_translated_post_type( $post_type ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'The post type "%s" is not enabled for translation in Polylang.', 'ai-translate-abilities' ),
					$post_type
				)
			);
		}
	...
	private static function translate_meta_value( $value, string $to, string $from = 'en' ) {
		if ( is_string( $value ) ) {
			if ( '' === trim( $value ) ) {
				return $value;
			}

			return self::translate( $value, $to, $from );
		}

		if ( is_array( $value ) ) {
			$translated_value = array();
			foreach ( $value as $key => $item ) {
				$translated_item = self::translate_meta_value( $item, $to, $from );
	...
	private static function translate_meta_value_for_key( string $meta_key, $value, string $to, string $from = 'en' ) {
		if ( 'slim_seo' === $meta_key && is_array( $value ) ) {
			...
			foreach ( array( 'title', 'description' ) as $field ) {
				...
				$translated_field = self::translate_meta_value( $translated_value[ $field ], $to, $from );
		...
		return self::translate_meta_value( $value, $to, $from );
	}
	 ...
	 */
	public static function translate_post( $post_id, $to, $status = '', $overwrite = false, $translate_title = true ) {
		$adapter = self::get_adapter();
		if ( ! $adapter ) {
			return new \WP_Error( 'no_translation_plugin', __( 'No supported translation plugin is active.', 'ai-translate-abilities' ) );
		}
		...
		if ( ! current_user_can( 'edit_post', $post_id ) ) {
			return new \WP_Error( 'forbidden_post', __( 'You are not allowed to translate this content item.', 'ai-translate-abilities' ) );
		}
		$post_type_check = self::validate_translatable_post_type( $post->post_type );
		if ( is_wp_error( $post_type_check ) ) {
			return $post_type_check;
		...
		if ( '' !== $from && $from === $to ) {
			return new \WP_Error( 'same_language', __( 'Source and target languages must be different.', 'ai-translate-abilities' ) );
		}
		...
		if ( $existing_translation && get_post_status( $existing_translation ) !== false && ! $overwrite ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'A translation for language "%1$s" already exists (post %2$d).', 'ai-translate-abilities' ),
					$to,
		...
		if ( $existing_translation && ! current_user_can( 'edit_post', $existing_translation ) ) {
			return new \WP_Error( 'forbidden_translation', __( 'You are not allowed to update the existing translation.', 'ai-translate-abilities' ) );
		}
		...
		// Translate content.
		$title   = $translate_title ? self::translate( $post->post_title, $to, $from ) : $post->post_title;
		$content = self::translate_post_content( $post->post_content, $to, $from );
		...
		foreach ( $meta as $key => $values ) {
			...
			if ( in_array( $key, self::meta_clear(), true ) ) {
				...
				$translated_meta = self::translate_meta_value_for_key( $key, $value, $to, $from );
